...
class ObjectDetector():
    def __init__(self, show, print):
        ...
        self.model = os.path.dirname(os.path.realpath(__file__)).replace("scripts", "models/citycocov2lgtclab_20.engine")
...
class InferenceModel:
    ...
    def prepare_input(self, image):
        ...
        # input_img = resize_with_padding(input_img, (self.input_width, self.input_height))
...
def computeMedianDepth(depthimg,box):
    ...
    if (index20percent%2) == 0:
        ...
        return (depths[int((index20percent-1)/2)] + depths[int((index20percent+1)/2)])/2
    
def distance_make_sense(distance,objectID,box):
        if distance>max_distance_thresholds[objectID]:
            return False
        else:
            height = box[3]-box[1]
            width = box[2]-box[0]
...
def resize_with_padding(input_image, target_size, background_color=(0, 0, 0)):
    """
    Resize an image while maintaining aspect ratio, adding padding as needed.

    Args:
    - input_image: The input image to resize.
    - target_size: A tuple (width, height) representing the target dimensions.
    - background_color: A tuple (B, G, R) representing the padding color.

    Returns:
    - The resized image with padding.
    """
    ...
    padded_image = cv2.copyMakeBorder(resized_image, top, bottom, left, right, cv2.BORDER_CONSTANT, value=background_color)
