...
class signFastest {
    public:
            ...
            :it(nh), align_to_color(RS2_STREAM_COLOR)
        {
            ...
            lane_pub = nh.advertise<utils::Lane>("/lane", 3);
            ...
            // if(hasDepthImage) {
            //     depth_sub = it.subscribe("/camera/aligned_depth_to_color/image_raw", 3, &signFastest::depthCallback, this);
            //     ros::topic::waitForMessage<sensor_msgs::Image>("/camera/aligned_depth_to_color/image_raw", nh);
...
int main(int argc, char **argv) {
    ...
    std::thread t1(&signFastest::runLane, &signFastest);
    std::thread t2(&signFastest::get_frames, &signFastest); // this already starts the thread i think
    // Spin ROS node
    while(ros::ok()) {
        // ros::spinOnce();
        signFastest.imageCallback();
        // loop_rate.sleep();
    }
    t2.join();
